...
| **Карты** | flutter_map + .mbtiles (OSM, оффлайн) |
...
| **Маркеры на карте** | Геозапросы (bbox/RTree), кластеризация, валидация координат | Хранение координат, описаний, категорий | flutter_map, дебаунс перемещения, кэш viewport, отрисовка | SQLite в PB не умеет PostGIS. Go вычисляет bbox |
...
| **Maps** | `flutter_map`, `mbtiles_provider`, `latlong2` | `^7.0.0`, `^1.0.0`, `^0.9.0` |
...
- ✅ Использовать `flutter_map` + `mbtiles_provider` для оффлайн-тайлов
